...
/// Precomputed pair-setup M2 material. Both modexps in M2 (the verifier
/// `v = g^x` and the server public key `B = k*v + g^b`) depend only on the
/// accessory pin and server randomness, never on the controller, so they can be
/// computed up front. On a slow CPU (e.g. the MT7628, where each 3072-bit
/// modexp is seconds) this is what keeps pair-setup under the controller's
/// timeout: M2 becomes a copy, and only the M4 modexp runs during the live
/// exchange. The same (salt, b) are reused across attempts, an acceptable
/// tradeoff for a single-pin home accessory.
#[derive(Clone)]
...
static SETUP_MATERIAL: std::sync::Mutex<Option<SetupMaterial>> = std::sync::Mutex::new(None);

fn compute_setup_material(pin: &str) -> SetupMaterial {
    let mut csprng = OsRng {};
    let mut salt = [0; 16];
    let mut b = [0; 64];
    csprng.fill_bytes(&mut salt);
    csprng.fill_bytes(&mut b);
    let private_key = srp_private_key::<Sha512>(b"Pair-Setup", pin.as_bytes(), &salt); // x = H(s | H(I | ":" | P))
    ...
    let srp_server =
        SrpServer::<Sha512>::new(&user, b"foo", &b, &G_3072).expect("SRP server from fresh material");
    let b_pub = srp_server.get_b_pub();
    SetupMaterial { salt, b, verifier, b_pub }
}

/// Precompute and cache pair-setup M2 material for `pin`, so the first pairing
/// does not pay the SRP cost during the live exchange. Call once at startup; it
/// runs two 3072-bit modexps (seconds on a slow CPU), so run it off the async
/// runtime.
pub fn precompute(pin: &crate::Pin) {
    let material = compute_setup_material(&pin.to_string());
    *SETUP_MATERIAL.lock().unwrap() = Some(material);
}
...
async fn handle_start(handler: &mut PairSetup, config: pointer::Config) -> Result<tlv::Container, tlv::Error> {
    ...
    // Use precomputed M2 material (kept warm since startup) so the two 3072-bit
    // modexps do not run during the live exchange; fall back to computing inline
    // if a pairing starts before warm-up finished.
    let material = {
        let cached = SETUP_MATERIAL.lock().unwrap().clone();
        match cached {
            Some(m) => m,
            None => {
                let pin = config.lock().await.pin.to_string();
                let m = compute_setup_material(&pin);
                *SETUP_MATERIAL.lock().unwrap() = Some(m.clone());
                m
            },
        }
    };
    let SetupMaterial { salt, b, verifier, b_pub } = material;
