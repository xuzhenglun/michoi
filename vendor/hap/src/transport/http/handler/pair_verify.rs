...
async fn handle_finish(
    ...
    match handler.session {
        ...
        Some(ref mut session) => {
            ...
            // A missing pairing must report Authentication, not Unknown: it is
            // the signal that tells the controller the accessory was reset so it
            // drops its stale pairing and offers to add it again. Letting the
            // io::NotFound map to Error::Unknown makes iOS retry verify forever.
            let pairing = storage
                .lock()
                .await
