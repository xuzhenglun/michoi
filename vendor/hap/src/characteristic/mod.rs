            ...
            if let Some(ref event_emitter) = self.event_emitter {
                event_emitter
                    ...
                    .emit(&Event::CharacteristicValueChanged {
                        ...
                        iid: self.id,
                        value: tlv8_json(self.format, json!(&val)),
                    })
                    .await;
            }
        }
...
/// HAP transports `tlv8` and `data` values as base64 strings, not as JSON
/// byte arrays (pad-gateway patch).
pub(crate) fn tlv8_json(format: Format, value: serde_json::Value) -> serde_json::Value {
    if matches!(format, Format::Tlv8 | Format::Data) {
        if let Some(bytes) = value.as_array() {
            let bytes: Vec<u8> = bytes.iter().filter_map(|b| b.as_u64().map(|b| b as u8)).collect();
            return json!(base64::Engine::encode(&base64::engine::general_purpose::STANDARD, bytes));
        }
    }
    value
}
...
impl<T: fmt::Debug + Default + Clone + Serialize + Send + Sync> Serialize for Characteristic<T> {
    fn serialize<S: Serializer>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error> {
        ...
        if self.perms.contains(&Perm::PairedRead) {
            // pad-gateway patch: tlv8/data values travel as base64 strings.
            let value = serde_json::to_value(&self.value).map_err(serde::ser::Error::custom)?;
            state.serialize_field("value", &tlv8_json(self.format, value))?;
        }
    ...
    for<'de> T: Deserialize<'de>,
{
    ...
    async fn get_value(&mut self) -> Result<serde_json::Value> {
        ...
        Ok(tlv8_json(self.format, json!(value)))
    }

    async fn set_value(&mut self, value: serde_json::Value) -> Result<()> {
        let v;
        let value = if matches!(self.format, Format::Tlv8 | Format::Data) && value.is_string() {
