...
| `008c/01` | Pad→楼宇网关 | 80/80 | — | 按 ID 呼叫远端门口机(经 192.168.120.2) |
...
## 7b. 主动查看(monitor)、按 ID 呼叫、电梯

来自 `call-door.cap`(Pad `.61` 主动访问 6 门口机 + 6 门铃,楼下门口机 `.3` 在第
...
对不在本子网的门口机/门铃(其他单元),Pad 向楼宇网关 `192.168.120.2` 发 `008c/01`,
端点块为 [本房间 | 目标门口机(IP 置 0,仅凭 ID)]。抓包里对 `M0005010{4,5,6}000`
和 `L0005011103{1..6}` 各试 10 次,均未响应(跨单元需网关侧路由/权限)。本仓库暂
不主动实现 008c,仅解析记录。
